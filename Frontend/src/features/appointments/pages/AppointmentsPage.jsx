...
function AppointmentsPage({ onNewAppointment }) {
  ...
  useEffect(() => {
    if (appointmentsByStatus.length === 0 || selectedStatusMonth) {
      return;
    }
  ...
  useEffect(() => {
    ...
    async function loadAppointmentsByStatus() {
      try {
        ...
        if (active) {
          setAppointmentsByStatusState('error');
        }
      }
    }
  ...
  useEffect(() => {
    if (monthlyAppointments.length === 0 || startDate || endDate) {
      return;
    }
  ...
  const detailStatuses = [...new Set(appointmentDetails.map((item) => item.estado))].sort();
  const detailSpecialties = [...new Set(appointmentDetails.map((item) => item.especialidad))].sort();
    ...
    <>
      ...
      <section className="dashboard-grid">
        ...
        <Panel className="span-4" title="Citas por estado">
          ...
          {appointmentsByStatusState === 'loading' && (
            <p className="panel-message">Cargando citas por estado...</p>
          )}
          {appointmentsByStatusState === 'error' && (
            <p className="panel-message error">No se pudo cargar las citas por estado.</p>
          )}
          {appointmentsByStatusState === 'success' && filteredAppointmentsByStatus.length === 0 && (
            <p className="panel-message">No hay datos disponibles para las citas por estado.</p>
          )}
          {appointmentsByStatusState === 'success' && filteredAppointmentsByStatus.length > 0 && (
        ...
        <Panel className="span-8" title="Citas por estado y mes">
          ...
          {monthlyStatus === 'error' && (
            <p className="panel-message error">No se pudo cargar el resumen mensual de citas.</p>
          )}
          {monthlyStatus === 'success' && filteredMonthlyAppointments.length === 0 && (
            <p className="panel-message">No hay datos disponibles para el resumen mensual.</p>
          )}
          {monthlyStatus === 'success' && filteredMonthlyAppointments.length > 0 && (
        ...
        <Panel className="span-12" title="Listado de citas">
          <Toolbar
            ...
            onStatusChange={(value) => {
              setDetailStatusFilter(value);
              setDetailPage(1);
            }}
            onSpecialtyChange={(value) => {
              setDetailSpecialtyFilter(value);
              setDetailPage(1);
            }}
          />
          {appointmentDetailsState === 'loading' && <p className="panel-message">Cargando listado de citas...</p>}
          {appointmentDetailsState === 'error' && (
            <p className="panel-message error">No se pudo cargar el listado de citas.</p>
          )}
          {appointmentDetailsState === 'success' && filteredAppointmentDetails.length === 0 && (
            <p className="panel-message">No hay citas para los filtros seleccionados.</p>
          )}
          {appointmentDetailsState === 'success' && filteredAppointmentDetails.length > 0 && (
            <>
              <DataTable
                ...
                rows={paginatedAppointmentDetails.map((appointment) => [
                  ...
                  <StatusBadge key={`${appointment.id_cita}-${appointment.especialidad}`} label={appointment.estado} />,
                  <RowActions key={`${appointment.id_cita}-${appointment.especialidad}-actions`} />,
                ])}
              />
              <div className="table-pagination">
                <span>
                  Pagina {safeDetailPage} de {totalDetailPages}
                </span>
                <div>
                  <button
